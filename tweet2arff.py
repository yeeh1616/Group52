...
def main():
    ...
    fields = [
        eca_old.arff.Field('tweet', eca_old.arff.Text()),
        eca_old.arff.Field('@@class@@', eca_old.arff.Nominal(['a', 'b', 'c']))
    ]

    # create item generator
    items = rows(args.file)

    eca_old.arff.save(args.output, fields, items, name='ARFF for ' + args.file.name)
