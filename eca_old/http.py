...
# (now you can just import eca_old.http and have access to all standard handlers)
...
class HelloWorld(httpd.Handler):
    ...
    def handle_GET(self):
        ...
        output = "<!DOCTYPE html><html><body><h1>Hello world!</h1><p><i>eca_old-session:</i> {}</p></body></html>"

        try:
            if not hasattr(self.request, 'cookies'): raise KeyError()
            cookie = self.request.cookies['eca_old-session'].value
        except KeyError:
